...
In the light of the results obtained within this study it is recommended to run _de novo_ assembly with Unicycler on hybrid read data. Thereby, a long read depth of coverage of 10 to 20 was shown to be sufficient in order to obtain highly complete, contiguous and correct assemblies, if the most recent base callers were used for read generation. The long read coverage should be increased appropriately if the quality of the long read data is lower, a coverage of $60$ should be sufficient to obtain high quality assemblies from long reads featuring a Phred quality of about 17.
***
...
- To control the quality of the input reads before conducting the assemblies, FastQC was run on all read sets. One _.html_ and one _.zip_ file is generated for each long read and both short read files as _./results/fastqc/\<SAMPLE-ID\>/\<READ-FILENAME\>_fastqc(.zip or .html)_.
